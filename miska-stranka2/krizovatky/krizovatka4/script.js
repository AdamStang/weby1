...
function animelektricka2(x) {
	...
	x.classList.remove("stopelektricka2");
		...
		var spravne_poradie = ["auto44","auto11","auto22"];  // pole so spravnym poradim
		var spravne_poradie2 = ["auto44","auto11","auto33"];  // pole so spravnym poradim
		...
		var element3 = document.getElementById('auto33'); // grab a reference to your element
		element3.addEventListener('click', clickHandler3); // event listener pre 3. auto
		
		var element4 = document.getElementById('auto44'); // grab a reference to your element
...
function sleep(ms) {
		...
		function demo() {
			...
			var c = document.getElementById("auto4");

			animauto4(c);
			sleep(1400).then(() => { 	animauto1(a); });
			sleep(4000).then(() => { 	animauto2(b); });
		}	
