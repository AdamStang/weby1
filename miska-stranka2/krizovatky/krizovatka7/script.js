...
function animelektricka2(x) {
	...
	x.classList.remove("stopelektricka2");
		...
		var spravne_poradie = ["auto1","auto2","auto4","auto3"];  // pole so spravnym poradim
		var spravne_poradie2 = ["auto2","auto1","auto4","auto3"];  // pole so spravnym poradim
...
function sleep(ms) {
		...
		function demo() {
			...
			sleep(3200).then(() => { 	animauto4(d); });
			sleep(4800).then(() => { 	animauto3(c); });
		}	
